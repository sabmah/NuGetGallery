        ...
        : AppController
    {
        ...
        public async virtual Task<ActionResult> UploadPackage()
        {
            ...
            using (var uploadedFile = await _uploadFileService.GetUploadFileAsync(currentUser.Key))
            {
                if (uploadedFile != null)
                {
                    ...
                    {
                        ActionsRequiringPermissions.UploadNewPackageId.TryGetAccountsAllowedOnBehalfOf(currentUser, new ActionOnNewPackageContext(packageMetadata.Id, _reservedNamespaceService), out accountsAllowedOnBehalfOf);
                    }
                    else
                    {
                        ActionsRequiringPermissions.UploadNewPackageVersion.TryGetAccountsAllowedOnBehalfOf(currentUser, existingPackageRegistration, out accountsAllowedOnBehalfOf);
                    }

                    if (!accountsAllowedOnBehalfOf.Any())
                    {
        ...
        public virtual async Task<JsonResult> UploadPackage(HttpPostedFileBase uploadFile)
        {
            ...
            using (var uploadStream = uploadFile.InputStream)
            {
                ...
                if (existingPackageRegistration == null && !ActionsRequiringPermissions.UploadNewPackageId.TryGetAccountsAllowedOnBehalfOf(currentUser, new ActionOnNewPackageContext(id, _reservedNamespaceService), out accountsAllowedOnBehalfOf))
                {
                    ModelState.AddModelError(
                ...
                if (existingPackageRegistration != null && !ActionsRequiringPermissions.UploadNewPackageVersion.TryGetAccountsAllowedOnBehalfOf(currentUser, existingPackageRegistration, out accountsAllowedOnBehalfOf))
                {
                    ModelState.AddModelError(
        ...
        public virtual async Task<ActionResult> DisplayPackage(string id, string version)
        {
            ...
            if (package == null
                || ((package.PackageStatusKey == PackageStatus.Validating
                    ...
                    && !ActionsRequiringPermissions.DisplayPrivatePackageMetadata.TryGetAccountsAllowedOnBehalfOf(currentUser, package, out accountsAllowedOnBehalfOf)))
            {
                return HttpNotFound();
            }
            ...
            if (ActionsRequiringPermissions.EditPackage.TryGetAccountsAllowedOnBehalfOf(currentUser, package, out accountsAllowedOnBehalfOf))
            {
        ...
        public virtual ActionResult ReportAbuse(string id, string version)
        {
            ...
            if (Request.IsAuthenticated)
            {
                ...
                if (ActionsRequiringPermissions.ReportPackageAsOwner.TryGetAccountsAllowedOnBehalfOf(user, package, out var accountsAllowedOnBehalfOf))
                {
        ...
        public virtual async Task<ActionResult> ReportMyPackage(string id, string version)
        {
            ...
            if (!ActionsRequiringPermissions.ReportPackageAsOwner.TryGetAccountsAllowedOnBehalfOf(GetCurrentUser(), package, out var accountsAllowedOnBehalfOf))
            {
        ...
        public virtual ActionResult ManagePackageOwners(string id)
        {
            ...
            if (!ActionsRequiringPermissions.ManagePackageOwnership.TryGetAccountsAllowedOnBehalfOf(GetCurrentUser(), package, out var accountsAllowedOnBehalfOf))
            {
        ...
        public virtual ActionResult Delete(string id, string version)
        {
            ...
            if (!ActionsRequiringPermissions.UnlistOrRelistPackage.TryGetAccountsAllowedOnBehalfOf(GetCurrentUser(), package, out var accountsAllowedOnBehalfOf))
            {
                return new HttpStatusCodeResult(401, "Unauthorized");
            }
        ...
        public virtual async Task<ActionResult> Edit(string id, string version)
        {
            ...
            if (!ActionsRequiringPermissions.EditPackage.TryGetAccountsAllowedOnBehalfOf(GetCurrentUser(), package, out var accountsAllowedOnBehalfOf))
            {
